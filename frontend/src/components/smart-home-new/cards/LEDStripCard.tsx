...
import type { DeviceCardProps, LEDStripState } from '@/types/smart-home-new';
  ...
  return (
    <div className="relative">
      <div className="absolute inset-0 h-2 top-1/2 -translate-y-1/2 rounded-full bg-white/10 overflow-hidden pointer-events-none">
        <div className="h-full rounded-full bg-gradient-to-r from-primary/60 to-primary" style={{ width: `${percentage}%` }} />
      </div>
      <input type="range" min="0" max="254" value={localValue} onChange={handleChange} disabled={disabled}
...
/**
 * LED Strip card — controls Line 2 only.
 */
export function LEDStripCard({ device, state, isConnected, onPublish }: DeviceCardProps) {
  ...
  const toggleState = useCallback(() => {
    onPublish({ state_l2: isL2On ? 'OFF' : 'ON' });
  }, [isL2On, onPublish]);

  const handleBrightnessChange = useCallback((value: number) => {
    if (value > 0 && !isL2On) {
      onPublish({ state_l2: 'ON', brightness_l2: value });
    } else if (value === 0 && isL2On) {
      onPublish({ state_l2: 'OFF' });
    } else {
      onPublish({ brightness_l2: value });
    }
  }, [onPublish, isL2On]);

  return (
    <DeviceCard name={device.name} size={device.size} isOn={isL2On} isOffline={device.isOffline} icon={<LineSegments size={20} weight="fill" />}>
      <div className="space-y-3">
        ...
        <BrightnessSlider value={displayBrightness} onChange={handleBrightnessChange} disabled={!isConnected || device.isOffline} />
        <button
          ...
          className={`
            ...
            ${isL2On ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-white/5 text-muted border border-white/5 hover:bg-white/10'}
