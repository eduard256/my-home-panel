...
import type { DeviceConfig, PresenceZoneState } from '@/types/smart-home-new';
 ...
 * Subscribes to each zone topic directly via smartHomeWs.subscribe().
 *
 * Layout:
 * ┌─────────┬─────────┐
 * │   Bed   │ Entrance│
...
export function PresenceZonesCard({
  ...
  useEffect(() => {
    ...
    const unsubscribes = device.presenceZones.map((zone, index) => {
      return smartHomeWs.subscribe(zone.topic, (state) => {
        ...
        if (newOccupancy) {
          ...
          // Debounce turning off - wait before turning off
